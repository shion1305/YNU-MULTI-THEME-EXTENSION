        ...
        for (const elt of elements) {
            ...
            if (id != null && (targetTypes.some(t => id.includes(t)))) {
                ...
                if (isOpen && notCompleted) {
                    ...
                    const deadlineDate = extractDeadline(submitStatus);
                    const deadlineTime = target.getElementsByTagName("a")[1].title.slice(-8,-3);
                    const deadline = deadlineDate + '/' + deadlineTime;
                    // 提出期限までの日数が15日以上ある場合は追加しない
                    if (!checkDeadlineWithinTwoWeeks(deadlineDate)) break;
...
function injectTable(homeworks) {
    ...
    if (homeworks.length == 0) {
        ...
        //①要素を変数保持
        let sortedHomeworks = Object.keys(homeworks).map(function(key) {
        return homeworks[key];
        //②dateでソート
        }).sort(function(a, b) {
        return (a.deadline < b.deadline) ? -1 : 1;  //オブジェクトの昇順ソート
        });
